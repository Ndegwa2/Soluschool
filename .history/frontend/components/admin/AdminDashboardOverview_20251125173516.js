...
const AdminDashboardOverview = () => {
  ...
  const { data: visitors } = useSWR('/api/visitors?status=pending&limit=100', fetcher)
  const { data: blacklisted } = useSWR('/api/visitors/blacklist?limit=100', fetcher)

  // Calculate real-time stats from API data
  const pendingCount = visitors?.total || 0
  const blacklistedCount = blacklisted?.total || 0
  
  const stats = [
    {
      title: 'Visitors Today',
      value: analytics?.total_pickups || 128,
      color: '#1E3A8A',
      icon: '👥'
    },
    {
      title: 'Currently Inside',
      value: analytics?.visitors || 27,
      color: '#FACC15',
      icon: '🚪'
    },
    {
