...
export default function Scan() {
  ...
  // Set default gate when gates are loaded
  useEffect(() => {
    if (gates?.gates && gates.gates.length > 0 && !selectedGate) {
      // Look for a "main" gate first, otherwise use the first one
      const mainGate = gates.gates.find(g => g.name.toLowerCase().includes('main')) || gates.gates[0]
      setSelectedGate(mainGate.id)
    }
  }, [gates, selectedGate])
