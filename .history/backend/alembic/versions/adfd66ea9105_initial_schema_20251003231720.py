...
def upgrade() -> None:
    ...
    # Since SQLite doesn't support adding FK after creation, we need to recreate the table or use batch mode.
    # For simplicity, let's drop and recreate users with FK.
    op.drop_table('users')
    op.create_table('users',
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    sa.Column('role', sa.String(length=50), nullable=False),
