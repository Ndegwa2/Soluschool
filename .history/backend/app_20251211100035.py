...
from models import User, Child, School, QRCode, Gate, Log, Notification, Visitor, VisitorPass, AuditLog
...
from socketio_utils import setup_socketio_handlers, create_notification, send_visitor_alert, send_security_alert
setup_socketio_handlers(socketio, db)
CORS(app)
...
# Module-level audit storage for fallback
audit_lock = threading.Lock()
audit_logs = []

# Enhanced persistent audit logging
def audit_log(action, user_id=None, entity_type=None, entity_id=None, details=None, request_info=None):
    """Comprehensive audit logging function using persistent storage"""
    try:
        ...
        sql_query = text("""
            INSERT INTO audit_logs (action, user_id, entity_type, entity_id, details, ip_address, user_agent, endpoint, session_id, timestamp)
            VALUES (:action, :user_id, :entity_type, :entity_id, :details, :ip_address, :user_agent, :endpoint, :session_id, CURRENT_TIMESTAMP)
        """)
        
        # Execute the insert with a separate connection to avoid transaction conflicts
        ...
        app.logger.info(f'AUDIT: {json.dumps({
            "id": audit_id,
            "action": action,
            "user_id": user_id,
            "entity_type": entity_type,
        ...
        # Fallback to memory storage if DB fails - don't break the main transaction
        try:
            with audit_lock:
                memory_entry = {
                    'id': len(audit_logs) + 1,
                ...
                audit_logs.append(memory_entry)
                app.logger.warning(f'AUDIT (MEMORY FALLBACK): {json.dumps(memory_entry)}')
        except Exception as fallback_error:
            app.logger.error(f'Failed to write audit log to memory fallback: {fallback_error}')
        
        # Don't re-raise the exception - let the main transaction continue
        return None
...
def analytics_summary():
    ...
    # Count unique visitors by counting distinct child records through QR codes or manual entries
    visitor_query = db.session.query(func.count(func.distinct(Log.id))).join(Gate).filter(Log.status == 'approved', Log.timestamp >= start)
    if school_id:
        visitor_query = visitor_query.filter(Gate.school_id == school_id)
    elif user.role == 'guard':
        visitor_query = visitor_query.filter(Gate.school_id == user.school_id)
    
    visitors = visitor_query.scalar()
...
def get_schools():
    ...
    app.logger.info(f'GET /api/schools called with admin_id: {admin_id}')
    ...
    if admin_id:
        ...
        try:
            auth_header = request.headers.get('Authorization')
            app.logger.info(f'Auth header present: {bool(auth_header)}')
            ...
            if not user_id:
                app.logger.warning('No user ID from JWT')
                return jsonify({'success': False, 'error': 'Invalid or missing token'}), 401

            user = User.query.get(user_id)
            app.logger.info(f'User found: {bool(user)}, role: {user.role if user else None}')
            ...
            if user.role != 'admin':
                app.logger.warning(f'User {user_id} with role {user.role} tried to access admin-only endpoint')
                return jsonify({'success': False, 'error': 'Admin only'}), 403
        ...
        app.logger.info(f'Found {len(schools)} schools for admin {admin_id}')
    else:
        # Public access for all schools
        schools = School.query.all()
        app.logger.info(f'Public access: returning all {len(schools)} schools')
...
def create_visitor():
    ...
    visitor_data = {
        ...
        'expected_arrival': visitor.expected_arrival.isoformat(),
        'host_name': visitor.host_name,
        'status': visitor.status
    }
    send_visitor_alert(visitor_data, 'created', db)
    audit_log(
...
def approve_visitor(visitor_id):
    ...
    try:
        ...
        # Send real-time notification after successful approval
        visitor_data = {
            'id': visitor.id,
            'name': visitor.name,
            'phone': visitor.phone,
        ...
        send_visitor_alert(visitor_data, 'approved', db)
        db.session.add(visitor_pass)
        db.session.flush()
...
if __name__ == '__main__':
    # Use SocketIO for real-time capabilities
