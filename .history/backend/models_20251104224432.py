...
class QRCode(db.Model):
    ...
    qr_data = Column(Text, unique=True, nullable=False)  # base64 encoded encrypted data
    qr_image = Column(Text, nullable=False)  # base64 encoded QR image
...
class Log(db.Model):
    ...
    visitor_pass_id = Column(Integer, ForeignKey('visitor_passes.id'))  # For visitor tracking
    ...
    visitor_pass = relationship('VisitorPass', back_populates='logs')
...
class Visitor(db.Model):
    ...
    id_number = Column(String(50))  # National ID or passport number
    company = Column(String(100))  # Visitor's company/organization
    purpose = Column(String(200), nullable=False)  # Purpose of visit
    expected_arrival = Column(DateTime, nullable=False)
    expected_departure = Column(DateTime)
    host_name = Column(String(100), nullable=False)  # Person they're visiting
    host_contact = Column(String(100))  # Host's phone/email
    status = Column(String(20), default='pending')  # pending, approved, denied, checked_in, checked_out
    is_blacklisted = Column(Boolean, default=False)
    blacklist_reason = Column(Text)
    created_by = Column(Integer, ForeignKey('users.id'))  # User who registered the visitor
    created_at = Column(DateTime, default=func.now())
    updated_at = Column(DateTime, default=func.now(), onupdate=func.now())

    # Relationships
    created_by_user = relationship('User', foreign_keys=[created_by])
    visitor_passes = relationship('VisitorPass', back_populates='visitor')
...
class VisitorPass(db.Model):
    ...
    gate_id = Column(Integer, ForeignKey('gates.id'), nullable=False)
    qr_code_data = Column(Text, nullable=False)  # Encrypted QR code data
    qr_image = Column(Text, nullable=False)  # Base64 encoded QR image
    ...
    logs = relationship('Log', back_populates='visitor_pass')

# Enhanced Log model to include visitor tracking
# Need to update the existing Log model to include visitor_pass_id
Index('idx_visitors_phone', Visitor.phone)
Index('idx_visitors_email', Visitor.email)
Index('idx_visitors_status', Visitor.status)
Index('idx_visitors_expected_arrival', Visitor.expected_arrival)
Index('idx_visitor_passes_visitor_id', VisitorPass.visitor_id)
Index('idx_visitor_passes_gate_id', VisitorPass.gate_id)
Index('idx_visitor_passes_expires_at', VisitorPass.expires_at)